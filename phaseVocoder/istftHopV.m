function x = istft(d, ftsize, w, hV)
...
if nargin < 3; w = 0; end
if nargin < 4; hV = 0; end  % will become winlen/2 later
...
xlen = round(ftsize + hV(end));
x = zeros(1,xlen);

b=0;
for iframe=1:size(d,2)
  b=hV(iframe);
  ft = d(:,iframe)';
  ft = [ft, conj(ft([((ftsize/2)):-1:2]))];
  px = real(ifft(ft));
  x((b):(b+ftsize-1)) = x((b):(b+ftsize-1))+px.*win;
  iframe=iframe+1;
end;
